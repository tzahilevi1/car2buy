...
(function () {
  ...
  if (wizard && window.Car2Buy) {
    ...
    // real render of the actual make+model — ONLY when a paid imagin.studio key is set (free tier watermarks; else falls back to the clean brand image)
    ...
    function realCarImg(makeHe, modelName) {
      if (!window.C2B_IMAGIN_KEY) return '';
      const mk = IMAGIN_MAKE[String(makeHe || '').trim()];
      const mf = String(modelName || '').toLowerCase().replace(/[^a-z0-9]+/g, '');
      if (!mk || !mf) return '';
      return 'https://cdn.imagin.studio/getimage?customer=' + encodeURIComponent(window.C2B_IMAGIN_KEY) + '&make=' + mk + '&modelFamily=' + encodeURIComponent(mf) + '&angle=23&width=560';
    }
